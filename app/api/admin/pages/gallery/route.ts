...
export async function GET() {
  try {
    ...
    try {
      const snap = await db.collection('pages').doc('gallery').get();
    ...
    console.error('GET /api/admin/pages/gallery error:', e);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.collection('pages').doc('gallery').set(body, { merge: true });

    return NextResponse.json({ ok: true }, { status: 200 });
  } catch (e: any) {
    console.error('POST /api/admin/pages/gallery error:', e);
    return NextResponse.json(
