...
export async function GET() {
  try {
    ...
    const docRef = db.collection('pages').doc('footer');
    ...
    console.error('GET /api/admin/pages/footer error:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.collection('pages').doc('footer').set(body, { merge: true });

    return NextResponse.json({ ok: true }, { status: 200 });
  } catch (e: any) {
    console.error('POST /api/admin/pages/footer error:', e);
    return NextResponse.json(
