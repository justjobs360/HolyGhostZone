...
export async function GET() {
  try {
    ...
    try {
      const snap = await db.collection('pages').doc('teachings').get();
    ...
    console.error('GET /api/admin/pages/teachings error:', e);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.collection('pages').doc('teachings').set(body, { merge: true });

    return NextResponse.json({ ok: true }, { status: 200 });
  } catch (e: any) {
    console.error('POST /api/admin/pages/teachings error:', e);
    return NextResponse.json(
